...
if 'extract_dir' not in st.session_state:
    st.session_state_extract_dir = tempfile.mkdtemp()
...
def extract_and_list_series(uploaded_file):
    ...
    # Dynamically find study folder (e.g., Head_Sempra_YYYYMMDD)
    study_folder = None
    for subdir in os.listdir(extract_path):
        subdir_path = os.path.join(extract_path, subdir)
        if os.path.isdir(subdir_path) and subdir.startswith('Head_'):
            study_folder = subdir_path
            break
    if not study_folder:
        st.error(f"No study folder (e.g., Head_Sempra_...) found in {extract_path}.")
        return series_groups, series_custom_names
